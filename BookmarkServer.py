...
def CheckURI(uri, timeout=10):
    ...
    try:
        ...
        if(r.status_code == 200):
            output = True
            print("URI response = %d" % r.status_code)
    except (ConnectionError, IOError) as err:
        print("URI {} not found, error => {}".format(uri, err))
    return output
...
class Shortener(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if name:
            if name in memory:
                ...
                # Not sure about this form of Location setting
                self.send_header('Location', '{}'.format(memory[name]))
                self.end_headers()
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            serv_feedback = "Please fill in all the fields in the form"

            self.wfile.write(form.format(serv_feedback).encode())
            return 
        ...
        if CheckURI(longuri):
            ...
            serv_feedback = "The URI is working and accepted"
            ...
            serv_feedback = "The URI specified could not be found"
